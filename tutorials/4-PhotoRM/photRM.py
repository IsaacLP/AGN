...
def plot_ccf_acf(delta, ccf, acf, locator=10, save=False, peak=False, tau=0, err=(0,0), lims_x=(-20,80), lims_y=(-0.25,1.25),title=None,time='days'):
    """
    ...
    ax1.plot(acf['tau'], acf['dcf'], 'o-',color='C2', markersize=3, linewidth=1, label='ACF')
    ax1.plot(ccf['tau'], ccf['dcf'], 'o--',color='C3', markersize=3, linewidth=1, label='CCF')
    ax1.set_xlabel("Time")
    ...
    if peak == True:
        ...
        ax2.vlines(tau, ymin=-1, ymax= delta['dcf'][nearest_idx],linestyles='dashed', colors='C0')
        ax2.axvspan(tau+err_low, tau+err_high, alpha=0.2)
    
    ax2.set_xlabel(f"Time ({time})", labelpad=8)
    ...
    if peak==True:
        if time=='days':
            ...
            unit = 'h'
        ax2.text(0.72,0.05, r'$\tau$ = {:.1f} ({:.1f},+{:.1f})'.format(tau,err_low,err_high) + unit, transform=ax2.transAxes, size=12.5)
    
    fig.text(0.03, 0.5, "Correlation (arbit. units)", va='center', rotation='vertical')
    if save==True:
